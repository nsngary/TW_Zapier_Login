...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            <h1 class="text-3xl font-bold text-neutral-800">新增產品</h1>
            <p class="text-neutral-600 mt-2">建立新的產品資料</p>
        </div>
        ...
        <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" class="p-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="status" 
                        ...
                        <option value="discontinued" {{ old('status') == 'discontinued' ? 'selected' : '' }}>停產</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="lg:col-span-2">
                    ...
                    <textarea id="description"
                              ...
                              placeholder="請輸入產品簡短描述">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="lg:col-span-2">
                    ...
                    <textarea id="detail"
                              ...
                              placeholder="請輸入產品詳細內容、規格、使用方法等">{{ old('detail') }}</textarea>
                    @error('detail')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="lg:col-span-2">
                    ...
                    {{-- 圖片預覽 --}}
                    <div id="images-preview" class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4 hidden"></div>
            ...
            <div class="flex items-center justify-end gap-3 mt-8 pt-6 border-t border-neutral-200">
                ...
                <button type="submit"
                    ...
                    <i class="fas fa-save mr-2"></i>建立產品
                </button>
            </div>
        </form>
    </div>
...
function handleImages(files) {
    ...
    files.forEach((file, index) => {
        ...
        reader.onload = function(e) {
            ...
            const isMainImage = imageFiles.length === 1; // 第一張圖片為主圖

            imageDiv.innerHTML = `
                <img src="${e.target.result}"
                     alt="圖片預覽"
                     class="w-full h-24 object-cover rounded-lg border-2 ${isMainImage ? 'border-primary-500' : 'border-neutral-200'} cursor-pointer hover:border-primary-400 transition-colors">
                <button type="button"
                        onclick="removeImage(this, ${imageFiles.length - 1})"
                ...
                ${isMainImage ?
                    '<div class="absolute bottom-1 left-1 bg-primary-500 text-white text-xs px-2 py-1 rounded">主圖</div>' :
                    '<button type="button" onclick="setAsMainImage(this)" class="absolute bottom-1 left-1 bg-neutral-800 hover:bg-neutral-900 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200">設為主圖</button>'
                }
            `;
...
function removeImage(button, fileIndex) {
    ...
    const wasMainImage = imageDiv.querySelector('.bg-primary-500');
    imageDiv.remove();

    // 如果刪除的是主圖且還有其他圖片，將第一張設為主圖
    if (wasMainImage && imageFiles.length > 0) {
        const previewContainer = document.getElementById('images-preview');
        const firstImage = previewContainer.querySelector('.relative');
        if (firstImage) {
            setAsMainImageElement(firstImage);
        }
    }
    ...
    remainingImages.forEach((img, index) => {
        const removeBtn = img.querySelector('button[onclick*="removeImage"]');
        if (removeBtn) {
            removeBtn.setAttribute('onclick', `removeImage(this, ${index})`);
        }
    });
...
function setAsMainImage(button) {
    ...
    setAsMainImageElement(imageDiv);
}

function setAsMainImageElement(imageDiv) {
    // 移除所有主圖標示
    document.querySelectorAll('#images-preview .relative').forEach(item => {
    ...
    const setMainBtn = imageDiv.querySelector('.bg-neutral-800');
    ...
    // 重新排列檔案陣列，將主圖移到第一位
    const previewContainer = document.getElementById('images-preview');
    const allImages = Array.from(previewContainer.querySelectorAll('.relative'));
    const mainImageIndex = allImages.indexOf(imageDiv);

    if (mainImageIndex > 0) {
        const mainImageFile = imageFiles[mainImageIndex];
        imageFiles.splice(mainImageIndex, 1);
